      ...
      <div class="row">
          ...
          <div class="col-md-12">
            <div class="box box-primary">
              <form id="applyLeaveForm" action="{{route('leaves.saveAppliedLeave')}}" method="POST" enctype="multipart/form-data">
                {{ csrf_field() }}
                  <div class="box-body">
                    @if($errors->any())
                  ...
                  @endif
                    ...
                    <div class="form-group">
                      <label>@lang('applyLeaveForm.hi.address') / @lang('applyLeaveForm.en.address')</label>
                      <textarea class="form-control text-capitalize" rows="3" name="address" id="address" placeholder="Please enter your address"></textarea>
                    </div>

                    <label class="al-file-box">@lang('applyLeaveForm.hi.chooseFile') / @lang('applyLeaveForm.en.chooseFile')
                      <input type="file" size="60" class="al-file" id="alChooseFileBtn" name="leaveDocuments[]" multiple="multiple"/>
                    </label>

                    <div id="fileList" class="al-selected-files"></div>
...
<script type="text/javascript">
  ...
  var employeeType = "{{$user->employee_type}}";
  var gender = "{{$gender}}";
  ...
  $('.selectTime').timepicker().on('changeTime.timepicker',function(e){
    ...
    if(Date.parse(fromTime) >= Date.parse(toTime)){
      ...
      if(leaveTypeId == 4){
          if(timeDiff < 8) {
              allowFormSubmit.time = 0;
              $(".timeErrors").text("You can not apply leave less than 8 hours.").css("color", "#f00");
              return false;
          }
      }
